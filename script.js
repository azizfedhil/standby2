...
function getWeatherInfo(code, isDay) {
    const weatherMap = {
        ...
        80: { desc: 'Slight Showers', icon: 'showers' },
        81: { desc: 'Moderate Showers', icon: 'showers' },
        82: { desc: 'Violent Showers', icon: 'showers' },
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                ...
                // Default to a fallback location (e.g., Cupertino) if permission is denied
                fetchWeatherData(37.323, -122.0322); 
            }
        );
    } else {
        console.error("Geolocation is not supported by this browser.");
        // Default to a fallback location if the feature is unavailable
        fetchWeatherData(37.323, -122.0322);
    }
}
...
function pixelRotation() {
    ...
    }, 5000); // Shits position every 5 seconds
}
...
// Set intervals to update data regularly
setInterval(updateTime, 1000); // Update the time every second
setInterval(getLocationAndWeather, 900000); // Update the weather every 15 minutes
